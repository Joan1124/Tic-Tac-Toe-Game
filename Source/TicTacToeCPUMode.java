...
public class TicTacToeCPUMode implements ActionListener {
	...
	public void formatCPUFile() { //Formats the cpuFile (saves a little bit of space since the formatter is used twice here and is also needed for the pause screen, so I added it as a function)
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(tttGUI.tictactoeFrame, "Error writing to " + cpuFile.getName() + " file", "File I/O Error", JOptionPane.WARNING_MESSAGE);
		...
		if (cpuFile.exists()) { //Checks if CPU Level.dat exists
			...
			try {
				...
				if (cpuLevel < cpuMinLevel || cpuLevel > cpuMaxLevel) { //Checks if the value is not in the range of accepted values for cpuLevel and changes the value if it is (both in the file and in the program)
					...
					JOptionPane.showMessageDialog(tttGUI.tictactoeFrame, "CPU Level has been set to its default value of " + cpuLevel + ". (Please use a value between " + cpuMinLevel + " and " + cpuMaxLevel + ")", "CPU Level Error", JOptionPane.WARNING_MESSAGE);
					//Displays a message for the user about the error
				}
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(tttGUI.tictactoeFrame, "Error reading " + cpuFile.getName() + " file", "File I/O Error", JOptionPane.WARNING_MESSAGE);
				//Displays a message for the user about the error
				
			} catch (NumberFormatException e) {
				
				e.printStackTrace();
				JOptionPane.showMessageDialog(tttGUI.tictactoeFrame, "Error with formatting of " + cpuFile.getName() + " file", "File I/O Error", JOptionPane.WARNING_MESSAGE);
				//Displays a message for the user about the error
				cpuLevel = 3; //Sets the cpuLevel back to 3
				formatCPUFile(); //Formats cpuFile
			...
			try {
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(tttGUI.tictactoeFrame, "Error creating " + cpuFile.getName() + " file", "File I/O Error", JOptionPane.WARNING_MESSAGE);
				//Displays a message for the user about the error
